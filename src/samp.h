...
int												getVehicleEmptySeat(vehicle_info* veh, bool passengerOnly = false);
...
void											addClientCommand(char *text, CMDPROC function, bool hidden = false);
...
int sampFindNearestPlayer(bool checkfr = false, bool checkdead = true);
int sampFindNearestDriver();
int getNearestEmptyVehicleID(float closestPlayerDist = 0.0);