...
CD3DFont				*pD3DFont = new CD3DFont( "Samsung Sans", 10, FCR_BORDER );

//pd3dFont_sampStuff = player info list, player score list, player ESP
CD3DFont				*pD3DFont_sampStuff = new CD3DFont( "Samsung Sans", 8.5, FCR_BORDER ); // 10 default

//pD3DFontFixed = cheat_state_msg, HUD
CD3DFont				*pD3DFontFixed = new CD3DFont( "Samsung Sans", 9, FCR_BORDER );

//pD3DFontFixedSmall = health under bars (cars, players), vehicle ESP
CD3DFont				*pD3DFontFixedSmall = new CD3DFont( "Small Fonts", 6.5, FCR_BORDER ); // 8 default

//pD3DFontChat = chat, kill list
//CD3DFont				*pD3DFontChat = new CD3DFont( "Tahoma", 10, FCR_NONE );
CD3DFont				*pD3DFontChat = new CD3DFont( "Samsung Sans", 11, FCR_BORDER );
...
void renderPlayerTags ( void )
{
	...
	if (set.default_nametags && cheat_state->render_player_tags && !set.tempnt_toggled)
	{
		set.tempnt_dist = g_SAMP->pSettings->fNameTagsDistance;
		set.tempnt_nt = g_SAMP->pSettings->byteShowNameTags;
		set.tempnt_walls = g_SAMP->pSettings->byteNoNametagsBehindWalls;

		g_SAMP->pSettings->fNameTagsDistance = 2000.0F;
		g_SAMP->pSettings->byteShowNameTags = 1;
		g_SAMP->pSettings->byteNoNametagsBehindWalls = 0;

		set.tempnt_toggled = 1;
	}

	if (set.tempnt_toggled && (!set.default_nametags || !cheat_state->render_player_tags || cheat_state->_generic.cheat_panic_enabled))
	{
		g_SAMP->pSettings->fNameTagsDistance = set.tempnt_dist;
		g_SAMP->pSettings->byteShowNameTags = set.tempnt_nt;
		g_SAMP->pSettings->byteNoNametagsBehindWalls = set.tempnt_walls;

		set.tempnt_toggled = 0;
	}

	// Exit this function and enable samp nametags, if panic key
	if ( cheat_state->_generic.cheat_panic_enabled || !cheat_state->render_player_tags || set.default_nametags)
	{
	...
	float		ESP_tag_player_D3DBox_pixelOffsetX = -0.43f; // -0.5f default
	float		ESP_tag_player_D3DBox_pixelOffsetY = 0.68f; // 1.0f default
	...
	while ( iter.pos < iter.end )
	{
		...
		if ( g_playerTagInfo[iGTAID].isStairStacked )
		{
			g_playerTagInfo[iGTAID].tagOffsetY -= 2.0f;
			g_playerTagInfo[iGTAID].stairStackedOffset -= 2.0f;
			if ( g_playerTagInfo[iGTAID].stairStackedOffset < 2.0f )
			{
	...
	while ( iter.pos < iter.end )
	{
		...
		while ( iterInner.pos < iterInner.end )
		{
						  ...
						  g_playerTagInfo[iGTAID_Inner].tagPosition.fY - g_playerTagInfo[iGTAID_Inner].tagOffsetY)) <= ESP_tag_player_espHeight )
			{
				...
				if ( g_playerTagInfo[iGTAID].tagPosition.fZ < g_playerTagInfo[iGTAID_Inner].tagPosition.fZ )
				{
					...
					g_playerTagInfo[iGTAID_Inner].tagOffsetY += 2.0f;
		...
		{
			if ( g_playerTagInfo[iGTAID].tagOffsetY >= 2.0f )
			{
				g_playerTagInfo[iGTAID].tagOffsetY -= 2.0f;
			}
			else
	...
	while ( iter.pos < iter.end )
	{
		...
		if ( g_Players )
		{
			...
			float drawX, drawY;
			drawX = g_playerTagInfo[iGTAID].tagPosition.fX; 
			drawY = playerBaseY - h;

			sprintf_s( buf, "%s (%d)", getPlayerName(iSAMPID), iSAMPID);
			pD3DFont_sampStuff->PrintShadow( drawX, drawY,
			samp_color_get( iSAMPID, 0xDD000000 ), buf );

			drawX += pD3DFont_sampStuff->DrawLength(buf);

			if (!g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iShowNameTag)
			{
				pD3DFont_sampStuff->PrintShadow( drawX, drawY,
					D3DCOLOR_XRGB(255, 0, 0), "  [M]");

				drawX += pD3DFont_sampStuff->DrawLength("  [M]");
			}

			if (std::find(Friends.begin(), Friends.end(), getPlayerName(iSAMPID)) != Friends.end() && Friends.size() > 0)
			{
				pD3DFont_sampStuff->PrintShadow(drawX, drawY,
					D3DCOLOR_XRGB(0, 0xFF, 0x33), "  [F]");

				drawX += pD3DFont_sampStuff->DrawLength("  [F]");
			}
...
void renderVehicleTags ( void )
{
	...
	while ( iter.pos < iter.end )
	{
		...
		if (iterVehicle->AreDoorsLocked() && set.lockedcaresp)
		{
			// get the camera
			...
			bool VehicleVisible = pGame->GetWorld()->IsLineOfSightClear(&src, iterVehicle->GetPosition(), true, false, false, true, true, false, false);

			float vPos[3] = { iterVehicle->GetPosition()->fX, iterVehicle->GetPosition()->fY, iterVehicle->GetPosition()->fZ };
			float camPos[3] = { src.fX, src.fY, src.fZ };

			if (vect3_dist(vPos, camPos) >= 70.0) VehicleVisible = false;

			if (VehicleVisible)
			{
				_snprintf_s(buf, sizeof(buf) - 1, "[LOCKED]");

				pD3DFontFixed->PrintShadow(screenPosition.fX, screenPosition.fY - h + ESP_tag_vehicle_pixelOffsetY + 2.5,
					D3DCOLOR_XRGB(255, 0, 0), buf);
			}
		}


		if (cheat_state->render_vehicle_tags) // Do only if vehicle tags are done
		{
			...
			// render vehicle name
			if (rwcolor.r <= 50 && rwcolor.g <= 50 && rwcolor.b <= 50)
			{
				rwcolor.r = 150;
				rwcolor.g = 150;
				rwcolor.b = 150;
			...
			pD3DFontFixed->PrintShadow(screenPosition.fX, screenPosition.fY - h + ESP_tag_vehicle_pixelOffsetY,
				D3DCOLOR_ARGB(224, rwcolor.r, rwcolor.g, rwcolor.b), buf);
			...
			vh /= 10.0f;
			if (vh > 50.0f)
				vcolor = D3DCOLOR_ARGB(128, 0, 150, 0);
			else if (vh > 25.0f)
				vcolor = D3DCOLOR_ARGB(128, 150, 150, 0);
			else
...
void proxyID3DDevice9_InitWindowMode ( D3DPRESENT_PARAMETERS *pPresentationParameters )
{
	...
	else if ( m_ForceUpdate && set.window_mode && !g_InitWindowMode_ForceUpdate_Active )
	{
		...
		*(int *)0xC920CC = 1;			// _?windowed
		...
		TwWindowSize(presentWidth, presentHeight);
		g_InitWindowMode_ForceUpdate_Active = false;
	}
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	if ( isBeginRenderWIN )
	{
			...
			if ( cheat_panic() || cheat_state->state == CHEAT_STATE_NONE )
			{
				...
				if( !set.flickering_problem )
				{
					if ( iIsSAMPSupported )
					{
						...
						_snprintf_s( buf, sizeof(buf)-1, "%s", NAME );
		...
		//if ( cheat_state->render_vehicle_tags ) we do it anyways now due to [LOCKED] showing update
			renderVehicleTags();
		...
		if (set.aimslapper == 1 && !isCheatPanicEnabled())
		{
			if (!g_Scoreboard->iIsEnabled)
			{
				float CenterX = (float)pPresentParam.BackBufferWidth / 2;
				float CenterY = (float)pPresentParam.BackBufferHeight / 2;
				render->D3DLine(CenterX + 15, CenterY + 15, CenterX + 3, CenterY + 3, D3DCOLOR_XRGB(253, 255, 12));
				render->D3DLine(CenterX - 15, CenterY + 15, CenterX - 3, CenterY + 3, D3DCOLOR_XRGB(253, 255, 12));
				render->D3DLine(CenterX + 15, CenterY - 15, CenterX + 3, CenterY - 3, D3DCOLOR_XRGB(253, 255, 12));
				render->D3DLine(CenterX - 15, CenterY - 15, CenterX - 3, CenterY - 3, D3DCOLOR_XRGB(253, 255, 12));
			}
		}
		if (set.atbmenu_init == 1)
		{
			if (set.atbmenu_enabled == 1)
			{
				if (!g_Scoreboard->iIsEnabled)
