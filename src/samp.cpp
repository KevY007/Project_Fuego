...
int getVehicleEmptySeat(vehicle_info* veh, bool passengerOnly)
{
	if (veh == NULL) return -1;

	int seat = -1;
	for (int i = 0; i < veh->m_nMaxPassengers; i++)
	{
		if (i == 0 && passengerOnly == true) continue;
...
actor_info* getVehicleActorInSeat(vehicle_info* veh, int seatID)
{
	if (veh == NULL) return NULL;

	return veh->passengers[seatID] != NULL ? veh->passengers[seatID] : NULL;
}
...
int getNearestEmptyVehicleID(float closestPlayerDist)
{
	struct vehicle_info* iGTAVeh = NULL;
	bool cancel = false;
	float fTempPos[3];
	float fTempVPos[3];

	float vDistances[SAMP_MAX_VEHICLES];
	float closestVehicleDistance = 69420.0f;
	int closestVehicle = 0xFFFF;

	for (int v = 0; v < SAMP_MAX_VEHICLES; v++)
	{
		vDistances[v] = 69420.0f;
		...
		iGTAVeh = getGTAVehicleFromSAMPVehicleID(v);

		if (iGTAVeh == NULL)
			continue;

		vect3_copy(&iGTAVeh->base.matrix[4 * 3], fTempVPos);

		for (int i = 0; i < iGTAVeh->m_nMaxPassengers; i++)
		{
			if (iGTAVeh->passengers[i] != NULL)
			{
				cancel = true;
				break;
			}
		...
		{
			if (!ValidPlayer(p) || !IsPlayerStreamed(p))
			{
				continue;
			}

			getPlayerPos(p, fTempPos);

			if (getDistanceFromTwoPoints(fTempVPos[0], fTempVPos[1], fTempVPos[2], fTempPos[0], fTempPos[1], fTempPos[2]) <= closestPlayerDist)
			{
				cancel = true;
				break;
			}
		...
		vDistances[v] = getDistanceFromLocalPlayer(fTempVPos[0], fTempVPos[1], fTempVPos[2]);
	}

	for (int v = 0; v < SAMP_MAX_VEHICLES; v++)
	{
		if (vDistances[v] >= 69420.0f || !ValidVehicle(v)) 
			continue;

		if (vDistances[v] <= closestVehicleDistance)
		{
			closestVehicle = v;
			closestVehicleDistance = vDistances[v];
		}
	}
...
int sampFindNearestPlayer(bool checkfr, bool checkdead)
{
	traceLastFunc("sampFindNearestPlayer()");

	if (!g_SAMP)
		return 65535;

	int iClosestPlayerID = 0xFFFF;
	float fClosestPlayerDistance = 694269.0f;
	float fDistances[SAMP_MAX_PLAYERS];

	CPed* iterPed = NULL;
	struct actor_info* targetact = NULL;

	traceLastFunc("sampFindNearestPlayer() --1");

	float fPos[3];

	for (int p = 0; p < SAMP_MAX_PLAYERS; p++)
	{
		fDistances[p] = 694269.0f;
		...
		if (checkfr && Friends.size() > 0 && std::find(Friends.begin(), Friends.end(), getPlayerName(p)) != Friends.end())
		{
			continue;
		}
		...
		if (checkdead) {
			...
			if (g_Players->pRemotePlayer[p]->pPlayerData->pSAMP_Actor->pGTAEntity->hitpoints < 0.9f) continue;
			targetact = getGTAPedFromSAMPPlayerID(p);
			if (targetact != NULL)
			{
				iterPed = pGameInterface->GetPools()->GetPed((DWORD*)targetact);

				if (g_Players->pRemotePlayer[p]->pPlayerData->fActorHealth < 0.9f) continue;

				if (iterPed->IsPlayingAnimation("SWEET_INJUREDLOOP")) continue;
				if (iterPed->IsPlayingAnimation("CS_DEAD_GUY")) continue;
				if (iterPed->IsPlayingAnimation("KO_SKID_FRONT")) continue;
				if (iterPed->IsPlayingAnimation("KO_SKID_BACK")) continue;
			}
		}

		if (getPlayerState(p) == PLAYER_STATE_NONE || getPlayerState(p) == PLAYER_STATE_WASTED)
		{
			continue;
		}
		...
		fDistances[p] = getDistanceFromLocalPlayer(fPos[0], fPos[1], fPos[2]);

		traceLastFunc("sampFindNearestPlayer() --5"); 
	}

	for (int p = 0; p < SAMP_MAX_PLAYERS; p++)
	{
		if (fDistances[p] >= 694269.0) continue;

		if (!ValidPlayer(p) || !IsPlayerStreamed(p))
			continue;

		if (fDistances[p] <= fClosestPlayerDistance)
		{
			iClosestPlayerID = p;
	...
	traceLastFunc("sampFindNearestPlayer() --6");
...
void addClientCommand(char *name, CMDPROC function, bool hidden)
{
	...
	if (g_m0dCmdNum < (SAMP_MAX_CLIENTCMDS - 22))
	{
		if (!hidden)
			...
			strncpy_s(g_m0dCmdlist[g_m0dCmdNum], "_", sizeof(g_m0dCmdlist[g_m0dCmdNum]) - 1);
...
int sampPatchDisableNameTags(int iEnabled)
{
	...
	if (iEnabled && !sampPatchEnableNameTags_patch.installed && !set.default_nametags)
