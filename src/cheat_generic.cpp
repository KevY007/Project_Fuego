...
DWORD cleanscreenshot = -1;

void cheat_handle_cleanscreenshot(void)
{
	if (KEY_DOWN(VK_F8) && cleanscreenshot == -1 && !isCheatPanicEnabled())
	{
		cheat_panic(true);

		cleanscreenshot = GetTickCount() + 1500;
	}
	if (GetTickCount() >= cleanscreenshot && cleanscreenshot != -1)
	{
		if (isCheatPanicEnabled()) cheat_panic(true);
		cheat_state_text("Clear Screenshot Taken");

		cleanscreenshot = -1;
	}
}

int cheat_panic ( bool forceToggle )
{
	...
	if ( KEYCOMBO_PRESSED(set.key_panic) || forceToggle == true)
	{
		// toggle panic
		...
		{
			last_atb_status = set.atbmenu_enabled;
			if (last_atb_status == 1)
			{
