...
void cheat_handle_vehicle_blinking_carlights ( 
	struct vehicle_info *vinfo, float time_diff )
{
...
void cheat_handle_vehicle_fly ( struct vehicle_info *vehicle_info, float time_diff )
{
	...
	if ( (!checked_for_orig_cheat && GTAPatchIDFinder( 0x00969160 ) != -1) || orig_cheat )
	{
		if ( !checked_for_orig_cheat )
		{
			...
			//Log( "Found gta patch \"Vehicles can fly\". Deactivating \"key_fly_vehicle\" and relating project_fuego functions." );
		}		
		return;
	}
	else
