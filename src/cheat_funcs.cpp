...
float getDistanceFromLocalPlayer(float fX, float fY, float fZ)
{
	...
	vect3_copy(&pSelfActor->base.matrix[4 * 3], fSelfPos);

	float fSX = (fX - fSelfPos[0]) * (fX - fSelfPos[0]);
	float fSY = (fY - fSelfPos[1]) * (fY - fSelfPos[1]);
	float fSZ = (fZ - fSelfPos[2]) * (fZ - fSelfPos[2]);

	return (float)sqrt(fSX + fSY + fSZ);
}

float getDistanceFromTwoPoints(float pX, float pY, float pZ, float fX, float fY, float fZ)
{
	float fSX = (fX - pX) * (fX - pX);
	float fSY = (fY - pY) * (fY - pY);
	float fSZ = (fZ - pZ) * (fZ - pZ);
...
int vehicle_find_nearest_ex(int flags, object_base *pos)
{
	struct vehicle_info* vehicle_self, * info;
	float					dist = -1.0f;
	int						id_nearest = -1;
	...
	for (int n = 0; n < pool_vehicle->size; n++)
	{
		...
		// looks like we're all good, now where the heck is we all at?
		vect3_vect3_sub(&pos->matrix[4 * 3], &info->base.matrix[4 * 3], vect);
		vect[2] += 2.0f;
		if (dist < 0.0f || vect3_length(vect) < dist)
		{
			dist = vect3_length(vect);
