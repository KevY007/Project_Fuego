...
void GTAfunc_CameraOnActor(actor_info *actor, int switchStyle)
{
	...
	if(pCamera && pEntity)
		pCamera->TakeControl(pEntity, MODE_FOLLOWPED, switchStyle);
}

void GTAfunc_CameraOnVehicle(vehicle_info *vehicle, int switchStyle)
{
	...
	CCamera *pCamera = pGameInterface->GetCamera();
	if(pCamera && pEntity)
		pCamera->TakeControl(pEntity, MODE_BEHINDCAR, switchStyle);
}
