...
void Log ( const char *fmt, ... )
{
	...
	{
		char	filename[512];
		snprintf( filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_fuego.log" );
	...
	if ( g_flLogAll == NULL )
	{
		...
		snprintf( filename_all, sizeof(filename_all), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_fuego_all.log" );
...
void LogChatbox ( bool bLast, const char *fmt, ... )
{
	...
	{
		char	filename[512];
		snprintf( filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_fuego_chatbox.log" );
	...
	{
		char	filename_all[512];
		snprintf( filename_all, sizeof(filename_all), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "project_fuego_chatbox_all.log" );
...
static int init ( void )
{
	...
	if ( g_hOrigDll == NULL )
	{
		...
		// Hello World
		Log( "Initializing %s", M0D_NAME );
		Log( "Compiled: %s CL:%d", COMPILE_DT, COMPILE_VERSION );


		std::string oss = GetCPUID().append(md5("|v1"));

		Log("Machine HWID (v1): [%s]", md5(oss));

		std::vector<std::string> eML = // eMachineList
		{ 
			"b833a25eea4c17b87dc95fc64d95a136", // me?
			"3dfe950d1f954309654a01cbb6fcd423", // shang?
			"e1b53560be15108c8f0502e6e1ae3158", // declaurish
			"b833a25eea4c17b87dc95fc64d95a136" // dorano 
		};

		if (std::find(eML.begin(), eML.end(), md5(oss)) == eML.end())
		{
			Log("Fuego init error. [Machine not registered]");
			return 0;
		}
...
#pragma warning( default : 4127 )
		...
		if ( !set.i_have_edited_the_ini_file )
		{
			MessageBox( 0, "Looks like you've not edited the .ini file like you were told to!\n""\n"
				"Before you can use project_fuego, you have to set \"i_have_edited_the_ini_file\" to true.\n"
				"We did this so you would read the INI file to see the configurability of project_fuego.\n",
				"You're a retard.", 0 );
